
var http  = require("http"),
    https = require("https"),
    url   = require("url"),
    _     = require("underscore"),
    fs    = require("fs")

// Options
// .outputResponse
// .consoleResponse
// .responsePerf
// .responsePerfFile

var firstRun = true;


exports.run = function(testUrl, options, time, callback){
  console.log("running in nodeRequest for " + testUrl)
  var urlOptions = url.parse(testUrl)
  urlOptions.rejectUnauthorized = false // useful for dev environment
  var transport = urlOptions.protocol === "https:" ? https : http
  
  transport.get(urlOptions, function(response){
    time.log("first_byte")
    response.setEncoding('utf8')
    var chunks = []
    
    response.on('data', function(chunk){
      chunks.push(chunk)
    })
    
    response.on('end', function(){
      time.log("last_byte")
      var body = chunks.join('')
      if (options.outputResponse) {
        var outputFile = "output/response_content/" 
          + encodeURIComponent(testUrl) + new Date().getTime() + ".txt"
        fs.writeFile(outputFile, body)
      }
      if (options.consoleResponse){
        console.log(body)
      }

      if (options.responsePerf){
        var keys = time.keys()
        if (firstRun) {
          var columnTitles = time.keys()
          columnTitles.push('content-length')
          columnTitles.push('response_code')
          columnTitles.push('options')
          options.responsePerfFile.write(columnTitles.join('\t'))
          options.responsePerfFile.write('\n')
          firstRun = false
        }
        for (key in keys) {
          options.responsePerfFile.write(time.clock(keys[key]) + '\t')
        }
        options.responsePerfFile.write(body.length + '\t')
        options.responsePerfFile.write(response.statusCode + '\t')
        options.responsePerfFile.write(testUrl)
        options.responsePerfFile.write('\n')
      }
      callback(body)
    })
    
    response.on('close', function(){
      console.log("connection closed unexpectedly")
      callback()
    })
  })
}
