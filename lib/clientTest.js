...
exports.runTest = function(testOptions, callback){
  ...
  if (repOptions.randomID) {
    ...
    repOptions.testUrl = replaceAll(repOptions.testUrl, 'RID', rand);
  }
  if (repOptions.randomLimit) {
    var rand = Math.floor(Math.random() * repOptions.randomLimitMax) + 1
    repOptions.testUrl = replaceAll(repOptions.testUrl, 'RLIMIT', rand);
  }
  if (repOptions.randomOffset) {
    var rand = Math.floor(Math.random() * repOptions.randomOffsetMax) + 1
    repOptions.testUrl = replaceAll(repOptions.testUrl, 'ROFFSET', rand);
  }
  if (repOptions.randomSelect) {
    ...
    repOptions.testUrl = replaceAll(repOptions.testUrl, "RSELECT", encodeURIComponent(selectColumns.join(', ')))
  }
  if (repOptions.selectRandomColumn) {
    ...
    repOptions.testUrl = replaceAll(repOptions.testUrl, "RCOLUMN", encodeURIComponent(selectColumn))
  }
