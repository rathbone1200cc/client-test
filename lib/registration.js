...
var registerAssertion = exports.registerAssertion = function(key, fn) {
  var assertWrapper = function(res,exp,testUrl) {
    try { fn(res,exp) } catch(e) { _assertFails+=1; console.log("FAILED ASSERT: " + testUrl + "\n" + e.message) }
  }
  return registerComponent("assertion", key, assertWrapper)
}
var registerComparator = exports.registerComparator = function(key, fn) {
  var compareWrapper = function(results,testUrls) { 
    try { fn(results) } catch(e) { _compareFails+=1; console.log("FAILED COMPARE:\n  " + testUrls.join('\n  ') + "\n" + e.message ) } 
  }
  return registerComponent("comparator", key, compareWrapper)
}
